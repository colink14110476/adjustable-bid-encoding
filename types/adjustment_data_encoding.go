...
// Hash: 3900f44ba70855d3fe59ae0ea78c50984253676c5b051908036ae441d91d4267
// Version: 0.1.3
package types
...
func (a *AdjustmentDataV2) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	for ii := 0; ii < len(a.CLPlaceholderTxProof); ii++ {
		offset += 4
		offset += len(a.CLPlaceholderTxProof[ii])
	}
	...
	if size := len(a.CLPlaceholderTxProof); size > 64 {
		err = ssz.ErrListTooBigFn("AdjustmentDataV2.CLPlaceholderTxProof", size, 64)
		return
	}
	{
		offset = 4 * len(a.CLPlaceholderTxProof)
		for ii := 0; ii < len(a.CLPlaceholderTxProof); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += len(a.CLPlaceholderTxProof[ii])
		}
	...
	for ii := 0; ii < len(a.CLPlaceholderTxProof); ii++ {
		if size := len(a.CLPlaceholderTxProof[ii]); size > 1073741824 {
			err = ssz.ErrBytesLengthFn("AdjustmentDataV2.CLPlaceholderTxProof[ii]", size, 1073741824)
			return
		}
...
func (a *AdjustmentDataV2) UnmarshalSSZ(buf []byte) error {
	...
	// Field (9) 'CLPlaceholderTxProof'
	{
		...
		a.CLPlaceholderTxProof = make([][]byte, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if len(buf) > 1073741824 {
				return ssz.ErrBytesLength
			}
			if cap(a.CLPlaceholderTxProof[indx]) == 0 {
				a.CLPlaceholderTxProof[indx] = make([]byte, 0, len(buf))
			}
			a.CLPlaceholderTxProof[indx] = append(a.CLPlaceholderTxProof[indx], buf...)
			return nil
		})
		if err != nil {
			return err
...
func (a *AdjustmentDataV2) SizeSSZ() (size int) {
	...
	for ii := 0; ii < len(a.CLPlaceholderTxProof); ii++ {
		size += 4
		size += len(a.CLPlaceholderTxProof[ii])
	}
...
func (a *AdjustmentDataV2) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		subIndx := hh.Index()
		num := uint64(len(a.CLPlaceholderTxProof))
		if num > 64 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range a.CLPlaceholderTxProof {
			{
				elemIndx := hh.Index()
				byteLen := uint64(len(elem))
				if byteLen > 1073741824 {
